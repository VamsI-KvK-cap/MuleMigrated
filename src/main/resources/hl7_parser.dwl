...
fun parse_hl7_message(str_message) {
  ...
  for (segment in segments) {
    ...
    if (segment_mapping[segment_name] != null) {
      var segment_fields = {};
      var field_mapping = segment_mapping[segment_name];

      for (field_name in field_mapping) {
        var field_index = field_mapping[field_name];
        segment_fields[field_name] = fields[field_index];
      }

      parsed_map[segment_name] = segment_fields;
    }
  }
